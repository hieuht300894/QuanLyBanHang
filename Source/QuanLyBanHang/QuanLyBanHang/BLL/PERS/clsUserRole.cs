...
namespace QuanLyBanHang.BLL.PERS
{
    public class clsUserRole
    {
        ...
        public static clsUserRole Instance
        {
            get
            {
                if (instance == null)
        ...
        public xUserFeature getUserFeature(string IDFeature)
        {
            ...
            if (clsGeneral.curAccount.IDPermission.HasValue)
               uf = db.xUserFeatures.FirstOrDefault(x => x.IDUserRole == clsGeneral.curAccount.IDPermission && x.IDFeature.Equals(IDFeature) && x.IsEnable);
